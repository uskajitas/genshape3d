...
app.post('/api/jobs/from-key', async (req, res) => {
  ...
  // Look up the text2image parent asset for this image key. If it
  // already has alt-view children, pass their URLs as auxImageUrls.
  // If the parent exists but has NO children yet, generate them on
  // the fly via Zero123++ (Replicate) so this 3D job actually gets
  // multi-view conditioning. The worker downloads everything and
  // feeds Hunyuan3D-2-mv. Requires REPLICATE_API_TOKEN in env.
  ...
  try {
    ...
    if (parentId) {
      ...
      if (auxImageUrls.length > 0) {
        console.log(`[jobs/from-key] attaching ${auxImageUrls.length} existing aux view(s) from parent ${parentId}`);
      } else if (process.env.REPLICATE_API_TOKEN) {
        // No children yet — generate them now via Zero123++. Synchronous
        // so the 3D job that follows actually gets to use them. This
        // adds ~30-60s to the request but only on first 3D from this
        // parent — every subsequent 3D job reuses the same children.
        console.log(`[jobs/from-key] parent ${parentId} has no aux views; auto-generating via Zero123++…`);
        try {
          const r2 = await getR2Stream(parentRow.image_key);
          const chunks: Buffer[] = [];
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          for await (const chunk of r2.Body as any) chunks.push(Buffer.from(chunk));
          const srcBytes = Buffer.concat(chunks);
          const srcMime  = r2.ContentType || 'image/jpeg';
          const mvViews  = await callMultiView(srcBytes, srcMime);
          console.log(`[jobs/from-key] MV provider returned ${mvViews.length} views for parent ${parentId}`);
          for (const v of mvViews) {
            const ext = v.contentType.includes('png') ? '.png' : '.jpg';
            const filename = `t2i-${v.label}-${Date.now()}${ext}`;
            const uploaded = await uploadToR2(v.bytes, filename, v.contentType);
            await createAsset({
              userEmail:     email,
              name:          `${parentRow.name || ''} (${v.label})`.trim(),
              prompt:        parentRow.prompt || '',
              finalPrompt:   parentRow.final_prompt || '',
              params:        parentRow.params || {},
              provider:      'replicate-zero123-auto',
              imageKey:      uploaded.key,
              seed:          null,
            ...
            auxImageUrls.push(`${publicUrl}/${uploaded.key}`);
          }
          console.log(`[jobs/from-key] auto-attached ${auxImageUrls.length} new aux view(s)`);
        } catch (mvErr: any) {
          console.warn(`[jobs/from-key] auto-multiview failed for parent ${parentId}: ${mvErr.message}`);
          // proceed with single-view path
        }
      } else {
        console.warn(`[jobs/from-key] parent ${parentId} has no aux views and REPLICATE_API_TOKEN is not set — skipping auto-multiview`);
      }
    }
  } catch (e: any) {
    console.warn(`[jobs/from-key] aux-view setup failed for key=${key}: ${e.message}`);
  }
